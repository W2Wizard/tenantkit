...
import { RESET_TOKEN_LENGTH, createResetToken } from "$lib/server/auth";
...
export const actions: Actions = {
	...
	reset: async ({ request, cookies, url, locals }) => {
		...
		await locals.context.lucia.invalidateUserSessions(token.userId);
		...
		const session = await locals.context.lucia.createSession(token.userId, {});
